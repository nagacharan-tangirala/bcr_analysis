...
with open(output_folder + 'bcr_agglomerative.pckl', 'rb') as i:
    [bcr_df, clusters_df] = pickle.load(i)

# Combine street params with the bcr and clusters df.
merged_df = bcr_df.merge(clusters_df, how='left', left_index=True, right_index=True)
merged_df = merged_df.merge(city_street_df, how='left', left_index=True, right_index=True)

n = 5
col = str(n) + '_clusters'

interested_columns = ['Sparse', 'Moderate', 'Dense', 'entropy', 'k_avg', 'edge_length_avg', 'edge_length_total', '5_clusters', 'area']
...
final_table = final_table.reindex([2, 1, 4, 3, 0])
final_table = final_table.applymap("{0:.2f}".format).astype(str)

# We can convert this table into latex table format by calling the helper function.
final_table_string = final_table.to_latex(index=False)

with open(output_folder + 'Tab2_bcr_clustering.txt', 'w') as o:
    o.write(final_table_string)
